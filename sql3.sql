...
--Задача 11Е (сложная)
--Самый популярный месяц для каждого читателя
--Таблицы:
--borrowers (id, name)
--loans (id, borrower_id, loan_date)
--📌 Условие:
--Для каждого читателя найти:
--Общее количество выдач книг
--Самый популярный месяц (когда читатель брал книги чаще всего)
--Количество выдач в этом месяце
--Дата первой и последней выдачи
--📋 Вывести:
--имя читателя
--общее количество выдач
--самый популярный месяц (название, например January)
--количество выдач в этом месяце
--первая дата выдачи
--последняя дата выдачи
--🔢 Отсортировать:
--по общему количеству выдач (убывание)
--показать топ-5 самых активных читателей
...
month_stats AS(
    SELECT borrower_id,
            ...
            COUNT(*) AS month_loans
    FROM loans
    GROUP BY borrower_id, MONTHNAME(loan_date)
),
top_months AS(
    SELECT borrower_id,
           month_name AS favorite_month,
           month_loans
    FROM month_stats ms1
    WHERE month_loans = (
        SELECT MAX(month_loans)
        FROM month_stats ms2
        WHERE ms1.borrower_id = ms2.borrower_id
    )
)
SELECT 
    rs.name,
    ...
    rs.first_loan,
    rs.last_loan
FROM readers_stats rs 
LEFT JOIN top_months tm ON rs.id = tm.borrower_id
ORDER BY rs.count_loans DESC;
LIMIT 5;
...
--Задача 15 (сложная)
--Самый продуктивный день для каждой библиотеки
--Таблицы:
--libraries (code, name, city)
--loans (id, library_code, loan_date)

--📌 Условие:
--Для каждой библиотеки найти:
--Общее количество выдач
--Самый продуктивный день (когда было больше всего выдач)
--Количество выдач в этот день
--Дата первой и последней выдачи
