...
Задача B1 (средняя)
Найти читателей, которые брали книги только одного жанра.
Вывести: имя читателя, жанр, количество взятых книг.
Отсортировать по количеству книг (убывание).

Задача B2 (сложная)
Для каждой библиотеки определить, какой процент от общего числа выдач составляют книги жанра 'Classics'.
Вывести: название библиотеки, город, процент (округлить до 1 знака).
Отсортировать по проценту (убывание).